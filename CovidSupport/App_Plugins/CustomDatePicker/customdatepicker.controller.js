'use strict';
function customDateTimePickerController($scope, angularHelper, dateHelper, validationMessageService) {
    var flatPickr = null;
    function onInit() {
        ...
        $scope.datePickerConfig = {
            ...
            time_24hr: !$scope.model.config.amPm
        };
    ...
    function setDatePickerVal() {
        if ($scope.model.value) {
            ...
            var amPmFormat = $scope.model.config.format;
            if ($scope.model.config.amPm) {
                amPmFormat = amPmFormat.replace("HH", "hh") + " A";
            }
            $scope.model.datetimePickerValue = dateVal.format(amPmFormat);
        } else {
            $scope.clearDate();
        }
    }
...
angular.module('umbraco').controller('Vizioz.PropertyEditors.CustomDatepickerController', customDateTimePickerController);